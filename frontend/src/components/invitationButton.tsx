...
export default function InvitationButton() {
    async function onClick() {
        ...
        console.log(user);
        // Get an edition
        //const editions: IEdition[] = await getAllEditionsFromUrl(apiPaths.registration);
        const editions = (await axios.get(apiPaths.editions, AxiosConf)).data._embedded.editions;
        ...
        const url = applicationPaths.registration + "?invitationToken="
            + postedInvitation.token;
    ...
    return (
        <Button onClick={onClick}>Create invitation for registration</Button>
    )
