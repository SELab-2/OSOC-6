...
async function getAllEntities<T extends IBaseEntity>(url:string, collectionName: string): Promise<T[]> {
...
async function getProjectAssignemntData():Promise<{ project: IProject; assignments: Assigments }[]> {
...
const ProjectAsignmentList: NextPage = () => {
    const [projects, setProjects] =
        useState<{ project: IProject; assignments: Assigments }[]>();
    ...
    return (
        <>
            <Container>
                <Accordion defaultActiveKey={["0"]} alwaysOpen className={"w-75 overflow-auto"} style={{'height': '20em'}}>
...
export function AssignmentItem( item:{ assignments: Assigments} ) {
    ...
    if (assign != undefined) {
        if (assign.length == 0) {
            return (
                <p>No students have been assigned to this project</p>
            )
        }
                        ...
                        <Container key={index}>
                            <Row className={"align-items-center"} xs={2} >
                                <Col sm={11}>
                                    ...
                                    <p>Suggested by {assignment.assigner.callName}: <br/> {assignment.assignment.reason}</p>
                                </Col>
                                <Col sm={1}>
                                    <CloseButton aria-label={"Remove student from project"} value={assignment.assignment._links.assignment.href} onClick={(assignment) => removeAssignment(assignment)}/>
                                </Col>
                            </Row>
                            <hr/>
                        </Container>
